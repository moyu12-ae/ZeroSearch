...
- **[G-BE-2]** 实现 COLD -> WARMING -> READY -> STALE -> DEAD 状态机，状态转换可观测
- **[G-BE-3]** 首次冷启动后浏览器常驻，后续搜索免冷启动（关联 [REQ-002]）
...
- **[G-BE-6]** 进程异常崩溃时自动恢复，SearchEngine 无感知
...
2. **冷启动慢**: Chrome 无头模式启动 ~2s，每个搜索会话需重启，累计延迟显著
...
| **HealthMonitor** | `browser_factory.py` (类 `HealthMonitor`) | 定时 `browser.is_connected()` 检查，异常自动重启 |
...
stateDiagram-v2
    ...
    COLD --> WARMING: get_context() 首次调用
    WARMING --> READY: Camoufox 启动完成 + 登录态验证
    WARMING --> COLD: launch 失败
    
    READY --> STALE: 心跳超时 (is_connected=false)
    READY --> DEAD: 浏览器进程崩溃
    READY --> READY: get_context() 命中
    
    STALE --> WARMING: 自动重连成功
    STALE --> COLD: 重连失败，重建
    
    DEAD --> COLD: 丢弃实例，下次调用重建
...
|------|---------|---------|---------|--------|
| `get_context()` 首次调用 | COLD | WARMING | 无 | 触发 `create_browser()` |
| `create_browser()` 成功 | WARMING | READY | Camoufox launch 无异常 | 记录启动耗时，`--debug` 打印 |
| `create_browser()` 失败 | WARMING | COLD | 异常抛出 | 清理残留进程 |
| `heartbeat` (is_connected=True) | READY | READY | 已连接 | 更新 `last_heartbeat` |
| `heartbeat` (is_connected=False) | READY | STALE | 超时未响应 | 触发 `_recover()` |
| `_recover()` 成功 | STALE | WARMING | 重新连接 | 无 Profile 丢失 |
| `_recover()` 失败 | STALE | COLD | 重连异常 | 清理进程,下次重建 |
| `on_crash` 回调 | READY | DEAD | 进程退出信号 | 设置 `_must_rebuild=True` |
| `get_context()` | DEAD | COLD | 标记重建 | 清理引用，准备重建 |
| `shutdown()` | 任意 | [*] | 主动调用 | 关闭进程，保存 Profile |
...
class BrowserContext:
    ...
    STATE_TIMEOUT = 300  # 5 分钟闲置视为 STALE
...
|------|---------|--------|---------|---------|---------|
| **`get_context()`** | 无 | `BrowserContext` (持有 `self._page`) | 每次搜索开始时调用 | `BrowserLaunchError` — Camoufox 未找到; `ProfileError` — Profile 损坏 | [REQ-001], [REQ-002] |
...
| **`shutdown()`** | 无 | `None` | Skill 退出/SIGTERM 时 | 无（尽力清理，不抛异常） | [REQ-002] (资源释放) |
| **`health_check()`** | 无 | `bool` | 内部心跳 / SearchEngine 调用 | 无（仅返回状态） | [REQ-002] |
...
sequenceDiagram
    ...
    alt 首次 / COLD
        ...
        BC->>BC: state: WARMING → READY
    else 已预热 / READY
        BC->>BC: health_check() → True
        BC-->>SE: 复用 context
    end
...
erDiagram
    ...
    BrowserContext {
        enum state "COLD|WARMING|READY|STALE|DEAD"
        object _browser "camoufox.Browser"
        object _page "camoufox.Page"
        float _last_used "最后使用时间戳"
    }
...
graph LR
    subgraph "预热常驻 (选择)"
        A1[首次搜索] -->|~2s| B1[浏览器进程存活]
        B1 -->|<0.5s| C1[后续搜索]
        B1 -->|5min 闲置| D1[自动 STALE]
    end
...
|------|---------|--------|
| P95 延迟 (非首次) | < 3s | ~5s |
| 内存占用 | ~200MB 常驻 | 按需 ~200MB |
| 进程管理复杂度 | 需要 HealthMonitor | 简单 |
| 崩溃恢复 | 需要 STALE/DEAD 状态 | 下次调用自动重启 |
| 适用场景 | 高频搜索（PRD 目标） | 低频单次搜索 |

**决策**: 预热常驻符合 PRD [REQ-002] 和 [REQ-005] (P95 <= 3s) 目标。通过 5 分钟闲置超时减少资源浪费，通过 HealthMonitor 处理崩溃恢复。
...
### 10.3 Trade-off 3: 主动关闭 vs 惰性关闭

**选择**: 惰性关闭（收到 SIGTERM 时关闭，平时常驻）。

| 维度 | 惰性关闭 | 每次搜索后关闭 |
|------|---------|-------------|
| 资源利用 | 5min 空闲才释放 | 每次释放 |
| 下次响应 | 免冷启动 < 0.5s | 需冷启动 ~2s |
| 进程泄漏风险 | 需 HealthMonitor | 风险低 |
| 实现复杂度 | 中等（需要状态机） | 低 |
...
根据 PRD [REQ-005]，全链路搜索需 <= 3s (预热后)。BrowserEngine 分得的时间预算：

| 环节 | 目标耗时 | 测量方式 | 负责组件 |
|------|---------|---------|---------|
| `get_context()` 命中 | < 1ms | state check | BrowserContext |
| `get_context()` 冷启动 | < 2000ms | `time.perf_counter()` | BrowserFactory |
| `navigate()` | < 1000ms | `page.goto` 计时 | Navigator |
| `wait_for_ai()` | < 1500ms | `page.wait_for_selector` 计时 | Navigator |
| `health_check()` | < 10ms | `is_connected()` 返回时间 | HealthMonitor |

**BrowserEngine 总预算**: 预热后 <= 2501ms，冷启动 <= 4000ms (在 PRD 8s 预算内)。注: 此预算与 SearchEngine §5.3.1 对齐。

### 12.2 性能优化策略

| 策略 | 描述 | 预期收益 |
|------|------|---------|
| **预热常驻** | 首次启动后保持进程，避免后续冷启动 | 省 ~2s/次 |
| **单 Page 复用** | 不新建 Page，复用已有 Tab | 省 ~200ms/次 |
| **导航超时优化** | `wait_until="domcontentloaded"` 替代 `"load"` | 省 ~500ms |
| **DNS 预取** | 首次搜索时预解析 `google.com` | 省 ~50ms |
| **Profile 预热** | 延迟加载扩展/插件 | 省 ~300ms (冷启动) |
...
| COLD | ~0MB | 无进程 |
| WARMING | ~150MB | 启动中峰值 |
| READY | ~200MB | Firefox 常驻 + 1 Tab |
| STALE | ~200MB | 进程存在但无响应 |
| DEAD | ~0MB | 已清理 |

**总预算**: BrowserEngine <= 250MB (含 Python 运行时)，符合本地 Skill 场景。
...
| `BrowserState` 转换 | COLD -> WARMING -> READY 合法性校验 |
...
| 测试场景 | 验证内容 |
|---------|---------|
| `create_browser` mock 成功 | BrowserContext 从 WARMING 转 READY |
| `create_browser` mock 失败 | BrowserContext 回退到 COLD |
| `health_check` 返回 False | 状态从 READY 转 STALE |
| `_try_reconnect` mock 成功 | STALE 恢复到 READY |
| `get_context()` 首次调用 | 触发 _cold_start |
| `get_context()` 二次调用 | 复用已有 context (不触发 _cold_start) |
...
| 进程崩溃自动恢复 | 手动 `kill` Firefox 后下次搜索自动重连 |
...
| 浏览器进程崩溃 | `on_crash` 回调 | 设置 `DEAD` 状态 → 下次调用重建 | 无感恢复 | 0 (恢复后继续) |
...
```text
[BrowserEngine] State: COLD → WARMING
...
[BrowserEngine] State: WARMING → READY
...
[BrowserEngine] Heartbeat OK (is_connected=True)
```
...
├── browser_factory.py       # BrowserContext, BrowserFactory, HealthMonitor, Navigator
...
| **HealthMonitor** | 心跳检测器，定时检查 `browser.is_connected()` |
| **COLD/WARMING/READY/STALE/DEAD** | 浏览器实例生命周期状态 |
