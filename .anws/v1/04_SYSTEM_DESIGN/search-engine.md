...
| ID | 目标 | 关联需求 |
|----|------|---------|
| G1 | 搜索全流程编排，从 CLI 参数解析到 Markdown 输出，P95 <= 3s (预热后) | [REQ-005] |
...
|------|------|---------|:------:|---------|
| `get_context()` | 无 | BrowserContext 对象 | 0ms (复用预热) / <2s (冷启动) | `BrowserStartError`, `ContextStaleError` |
...
| 浏览器 context 获取 | 0ms (复用) | 无 | BrowserEngine | 预热常驻，复用免获取 |
...
| **总计** | **<= 3000ms** | 端到端计时 | SearchEngine | --debug 输出各环节耗时 |

**冷启动特殊预算**: 首次搜索 (含 Firefox 冷启动) P95 <= 8000ms，额外 5s 分配给 BrowserEngine 初始化。

### 9.2 性能优化策略

1. **预热常驻**: 浏览器实例进程保持 alive (参见 [REQ-002])，context 获取环节节时 1-2s
...
[SearchEngine] 2026-05-19 10:30:15.456 | context_acquire: 0ms (reused)
...
[SearchEngine] 2026-05-19 10:30:18.313 | total: 3190ms (WARN: exceeded 3000ms budget)
```
...
**风险缓解**: 预热常驻策略下浏览器进程保持 alive 而不是每次都重启，缓存生命周期 > 单次搜索会话
