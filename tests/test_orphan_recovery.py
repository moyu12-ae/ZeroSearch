"""TDD: 孤儿进程恢复测试 + 平台抽象层端口/PID 测试

测试 BrowserFactory._find_and_recover_orphan() 和 platform.get_pid_on_port()
——幽灵连接根因修复的验证。
"""
...
class TestOrphanRecovery:
    ...
    def test_find_orphan_no_chrome_returns_none(self):
        """当没有 Chrome 在已知端口范围监听时，应跳过所有端口。"""
        from src.browser.browser_factory import BrowserFactory

        result = BrowserFactory._find_and_recover_orphan()
        # 注意：如果真 Chrome Daemon 已运行，result 可能是 tuple。
        # 此测试验证方法不抛异常且返回类型正确。
        assert result is None or isinstance(result, tuple), (
            f"返回值应为 None 或 (port, profile_path) tuple，实际: {type(result)}"
        )
    ...
    def test_find_orphan_port_not_cdp_returns_none(self):
        ...
        try:
            ...
            result = BrowserFactory._find_and_recover_orphan()
            # 如果 Chrome Daemon 运行在 9222，会找到它；或者没有则返回 None
            # 关键是 9225 上的普通 HTTP 被正确跳过
            if result is not None:
                port, _ = result
                assert port != 9225, f"不应将非 CDP 端口 9225 识别为 Chrome"
        finally:
            server.shutdown()
    ...
    def test_orphan_recovery_never_throws(self):
        ...
        try:
            result = BrowserFactory._find_and_recover_orphan()
            # 无 Chrome 时应返回 None，有 Chrome 时返回 tuple
            assert result is None or isinstance(result, tuple)
...
class TestPlatformGetPidOnPort:
    """平台模块 get_pid_on_port() 跨平台测试"""

    def test_get_pid_closed_port_returns_none(self):
        """对关闭的端口查询 PID 应返回 None。"""
        from src.utils.platform import get_pid_on_port
        pid = get_pid_on_port(19999)
        assert pid is None, f"关闭端口应返回 None，实际返回 {pid}"

    def test_get_pid_zero_port(self):
        """端口 0 是保留端口，应返回 None。"""
        from src.utils.platform import get_pid_on_port
        pid = get_pid_on_port(0)
        assert pid is None, f"端口 0 应返回 None，实际返回 {pid}"

    def test_get_pid_timeout_handled(self):
        """当 lsof/netstat 超时时应优雅降级返回 None。"""
        from src.utils.platform import get_pid_on_port
    ...
    def test_get_pid_permission_denied(self):
        """当因权限拒绝失败时应返回 None。"""
        from src.utils.platform import get_pid_on_port
        with patch("subprocess.run") as mock_run:
    ...
    def test_get_pid_returns_empty(self):
        """当 lsof/netstat 返回空输出时应返回 None。"""
        from src.utils.platform import get_pid_on_port
        with patch("subprocess.run") as mock_run:
            ...
            result = get_pid_on_port(9222)
            assert result is None, "空输出应返回 None"
...
class TestOrphanRecoveryEdgeCases:
    ...
    def test_orphan_recovery_port_scan_idempotent(self):
        ...
        assert all(type(r) is expected_type for r in results), (
            f"多次调用应返回相同类型，实际: {[type(r).__name__ for r in results]}"
        )
